...
def dt_ydb(val):
    """datetime → YottaDB compact string YYYYMMDDHHMMSS"""
    if val is None:
        return ""
    return str(val).replace("-", "").replace(" ", "").replace(":", "")[:14]

def dt_str(val):
    """datetime → plain string, empty if None"""
    if val is None:
        return ""
    return str(val)
...
def load_patients(ydb, pg):
    print("\n[1] Loading patients → ^PHD PID nodes")
    cur = pg.cursor()
    ...
    for row in cur:
        ...
        # Reverse lookup: MIMIC subject_id → canonical pid
...
def load_admissions(ydb, pg):
    print("\n[2] Loading admissions → ^PHD VISIT nodes")
    cur = pg.cursor()
    ...
    for row in cur:
        ...
        # Encounter → patient reverse index
    ...
    print(f"\n    ✓ {n:,} admissions loaded, {skipped} skipped (patient not found)")
    cur.close()
    return n
...
def load_icu(ydb, pg):
    print("\n[3] Loading ICU stays → ^PHD VISIT/ICU nodes")
    cur = pg.cursor()
    ...
    print(f"\n    ✓ {n:,} ICU stays loaded, {skipped} skipped")
    cur.close()
    return n
...
def load_diagnoses(ydb, pg):
    print("\n[4] Loading diagnoses → ^PHD VISIT/DX nodes")
    ...
    print(f"\n    ✓ {n:,} diagnoses loaded, {skipped} skipped")
    cur.close()
    return n
...
def load_procedures(ydb, pg):
    print("\n[5] Loading procedures → ^PHD VISIT/PROC nodes")
    ...
    print(f"\n    ✓ {n:,} procedures loaded, {skipped} skipped")
    cur.close()
    return n
...
def load_prescriptions(ydb, pg):
    print("\n[6] Loading prescriptions → ^PHD VISIT/MED nodes")
    cur = pg.cursor()
    ...
    print(f"\n    ✓ {n:,} prescriptions loaded, {skipped} skipped")
    cur.close()
    return n
...
def load_labs(ydb, pg):
    print("\n[7] Loading lab events → ^PHD VISIT/LAB nodes")
    cur_count = pg.cursor()
    ...
    print(f"\n    ✓ {n:,} labs loaded, {skipped} skipped, {capped:,} capped at {MAX_LABS}/encounter")
    cur.close()
    return n
...
def load_notes(ydb, pg):
    ...
    cur_count.execute("""
        ...
        WHERE hadm_id IS NOT NULL AND iserror IS DISTINCT FROM 1
    """)
    total = cur_count.fetchone()[0]
    print(f"    {total:,} notes to load (hadm_id not null, not error)")
    print(f"    Max {MAX_NOTES} notes per patient")

    # Track note index per patient
    pat_note_idx = {}
    ...
    cur.execute("""
        ...
        ORDER BY subject_id, charttime NULLS LAST, chartdate
    """)
    ...
    while True:
        rows = cur.fetchmany(5000)  # smaller chunks — notes are large
        if not rows:
            break
        ...
        for row in rows:
            ...
            note_dt = dt_ydb(charttime) if charttime else dt_ydb(chartdate)

            # Note metadata
            ydb.set("^PHD", [pid, "VISIT", eid, "NOTE", str(idx)],
                f"{safe(category, 50)}^{note_dt}^"
                f"{safe(str(cgid) if cgid else '', 10)}^"
                f"{safe(description, 100)}")

            # Note full text (can be large — YottaDB handles up to 1MB per node)
            if text:
                ydb.set("^PHD", [pid, "VISIT", eid, "NOTETXT", str(idx)],
                    str(text)[:500000])  # cap at 500KB per note
    ...
    print(f"\n    ✓ {n:,} notes loaded, {skipped} skipped, {capped:,} capped at {MAX_NOTES}/patient")
    cur.close()
    return n
...
STEPS = [
    ("patients",      load_patients,      "PATIENTS → ^PHD PID"),
    ("admissions",    load_admissions,    "ADMISSIONS → ^PHD VISIT"),
    ("icu",           load_icu,           "ICUSTAYS → ^PHD VISIT/ICU"),
    ("diagnoses",     load_diagnoses,     "DIAGNOSES_ICD → ^PHD VISIT/DX"),
    ("procedures",    load_procedures,    "PROCEDURES_ICD → ^PHD VISIT/PROC"),
    ("prescriptions", load_prescriptions, "PRESCRIPTIONS → ^PHD VISIT/MED"),
    ("labs",          load_labs,          "LABEVENTS → ^PHD VISIT/LAB"),
    ("notes",         load_notes,         "NOTEEVENTS → ^PHD VISIT/NOTE"),
]
...
def print_summary(results, elapsed):
    print("\n" + "═" * 55)
    print("  MIMIC-III → YottaDB Load Summary")
    print("═" * 55)
    ...
    for r in results:
        ...
        t       = f"{r['elapsed']:.1f}s"
        count   = f"{r['count']:,}" if r.get("count") else "—"
...
def main():
    parser = argparse.ArgumentParser(description="MIMIC-III → YottaDB ETL")
    ...
    print("=" * 55)
    print("  MIMIC-III → YottaDB ^PHD ETL")
    ...
    pg = get_pg()
    print("  ✓ Postgres connected (MIMICold)")
