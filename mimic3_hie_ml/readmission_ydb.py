"""
readmission_ydb.py
30-Day Hospital Readmission Prediction — MUMPS path
Reads features directly from YottaDB ^PHD globals
Same model as readmission_pg.py — compares speed
...
def extract_patient_features(ydb, pg):
    ...
    # Get readmission labels from Postgres — one query, then close
    ...
    print(f"\n  ✓ {n_patients:,} patients, {n_encounters:,} encounters traversed "
...
def train_and_evaluate(df, run_id, source):
    ...
    # Filter to index admissions only (those with a label)
    df_model = df[df["readmitted_30d"].notna()].copy()
...
def main():
    ...
    print("=" * 58)
    print("  30-Day Readmission — MUMPS Path")
    ...
    print("✓ Postgres connected (labels only)")
    ...
    print(f"  ✓ Feature extraction: {feat_time:.2f}s")
