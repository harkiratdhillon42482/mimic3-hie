"""
sync_engine.py — YottaDB ^PHD → Postgres hie schema
...
def yget(ydb, *args):
    """Safe get — returns empty string on any error."""
    try:
        val = ydb.get(*args)
        if val is None:
            return ""
...
def ynext(ydb, gbl, subs):
    """Safe subscript_next — returns None when exhausted."""
    try:
...
def sync_visits(ydb, pg, source_id, limit=None):
    print("\n[2] Syncing visits → hie.visit_occurrence")
    ...
    print(f"\n    ✓ {n_visits:,} visits, {n_icu:,} ICU stays synced")
    return n_visits
...
def sync_conditions(ydb, pg, source_id, limit=None):
    print("\n[3] Syncing conditions → hie.condition_occurrence")
...
def sync_procedures(ydb, pg, source_id, limit=None):
    print("\n[4] Syncing procedures → hie.procedure_occurrence")
...
def sync_drugs(ydb, pg, source_id, limit=None):
    print("\n[5] Syncing drugs → hie.drug_exposure")
...
def sync_measurements(ydb, pg, source_id, limit=None):
    print("\n[6] Syncing measurements → hie.measurement")
...
STEPS = [
    ("person",     sync_person,       "^PHD PID → hie.person"),
    ("visits",     sync_visits,       "^PHD VISIT → hie.visit_occurrence + visit_detail"),
    ("conditions", sync_conditions,   "^PHD DX → hie.condition_occurrence"),
    ("procedures", sync_procedures,   "^PHD PROC → hie.procedure_occurrence"),
    ("drugs",      sync_drugs,        "^PHD MED → hie.drug_exposure"),
    ("labs",       sync_measurements, "^PHD LAB → hie.measurement"),
    ("notes",      sync_notes,        "^PHD NOTE → hie.note"),
]
...
def print_summary(results, elapsed):
    print("\n" + "═" * 58)
    print("  ^PHD → Postgres HIE Sync Summary")
    print("═" * 58)
...
def main():
    parser = argparse.ArgumentParser(description="^PHD → Postgres HIE sync")
    ...
    print("=" * 58)
    print("  ^PHD → Postgres HIE Sync Engine")
    ...
    print(f"✓ Source ID: {source_id} (MIII)")
